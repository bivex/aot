...
# Script to rebuild German morphological dictionaries for RML project
# Usage: ./rebuild_german_dicts.sh [--clean] [--skip-build]

set -e

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
BUILD_DIR="${PROJECT_ROOT}/build"
DICT_SOURCE_DIR="${PROJECT_ROOT}/Source/morph_dict/data/German"
DICT_OUTPUT_DIR="${PROJECT_ROOT}/Dicts/Morph/German"
...
echo "=== RML German Dictionary Rebuild Script ==="
echo ""
...
if [[ "$SKIP_BUILD" != true ]]; then
    ...
    [[ ! -f "$MORPH_GEN" ]] && { echo "ERROR: morph_gen not found"; exit 1; }
    echo ""
fi

chmod +x "$MORPH_GEN" 2>/dev/null || true

echo "[5/6] Regenerating German morphological dictionaries..."
cd "${PROJECT_ROOT}"
cmake --build "${BUILD_DIR}" --target German_Morph -- -j$(sysctl -n hw.ncpu 2>/dev/null || nproc 2>/dev/null || echo 4)
...
echo ""
echo "Note: German WordData.txt is empty (no frequency corpus), so *wordweight.bin files will be empty."
echo "This is expected for the German dictionary in this project."
echo ""

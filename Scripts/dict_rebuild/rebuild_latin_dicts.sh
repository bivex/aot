...
DICT_SOURCE_DIR="${PROJECT_ROOT}/Source/morph_dict/data/Latin"
DICT_OUTPUT_DIR="${PROJECT_ROOT}/Dicts/Morph/Latin"
MORPH_GEN="${BUILD_DIR}/Source/morph_dict/morph_gen/morph_gen"
CONVERTER="${PROJECT_ROOT}/dev/lat_conv/unimorph_lat_to_aot.py"
UNIMORPH_DATA="${PROJECT_ROOT}/Dicts/Morph/Latin/unimorph/lat"
...
echo "=== RML Latin Dictionary Rebuild Script ==="
echo ""
...
if [[ "$SKIP_CONVERT" != true ]]; then
    echo "[2/7] Converting UniMorph Latin data..."
...
echo "[6/7] Regenerating Latin morphological dictionaries..."
echo "Source: ${DICT_SOURCE_DIR}"
echo "Output: ${DICT_OUTPUT_DIR}"
echo ""

cd "${PROJECT_ROOT}"
cmake --build "${BUILD_DIR}" --target Latin_Morph -- -j$(sysctl -n hw.ncpu 2>/dev/null || nproc 2>/dev/null || echo 4)

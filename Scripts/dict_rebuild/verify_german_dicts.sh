...
# Quick verification for German dictionaries
set -e

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
DICT_DIR="${PROJECT_ROOT}/Dicts/Morph/German"
SOURCE_DIR="${PROJECT_ROOT}/Source/morph_dict/data/German"

echo "=== RML German Dictionary Verification ==="
echo ""
...
REQUIRED_FILES=("morph.bases" "morph.annot" "morph.forms_autom" "gramtab.json" "morph.options" "lhomoweight.bin" "chomoweight.bin" "fhomoweight.bin" "lwordweight.bin" "cwordweight.bin" "fwordweight.bin" "npredict.bin")
MISSING=0
...
[[ $MISSING -gt 0 ]] && { echo ""; echo "ERROR: ${MISSING} missing."; exit 1; }
...
words=("wiederaufbau" "wiederherstellen" "rekonstruieren" "erneuern" "reparieren" "renovieren" "sanieren")
...
echo ""
echo "Note: German dictionaries lack frequency data (WordData.txt is empty)."
echo "Frequency files (*wordweight.bin) will be empty, which is normal."
echo ""

...
TSynanHttpServer::TSynanHttpServer() :
    ...
    FinnishSyntaxHolder(morphFinnish),
    ItalianSyntaxHolder(morphItalian),
    HungarianSyntaxHolder(morphHungarian),
    PolishSyntaxHolder(morphPolish)
{
...
std::string TSynanHttpServer::ProcessSyntax(TDaemonParsedRequest &request) {
    ...
    } else if (request.Langua == morphFinnish) {
        P = &FinnishSyntaxHolder;
    } else if (request.Langua == morphItalian) {
        P = &ItalianSyntaxHolder;
    } else if (request.Langua == morphHungarian) {
        P = &HungarianSyntaxHolder;
    } else if (request.Langua == morphPolish) {
        P = &PolishSyntaxHolder;
    }
    ...
    if (request.Langua == morphEnglish || request.Langua == morphSpanish || request.Langua == morphLatin || request.Langua == morphFrench || request.Langua == morphPortuguese || request.Langua == morphFinnish || request.Langua == morphItalian || request.Langua == morphHungarian || request.Langua == morphPolish) {
        ...
        if (request.Langua == morphFinnish) {
            StripFinnishAccents(query);
        }
        if (request.Langua == morphHungarian) {
            StripHungarianAccents(query);
        }
        if (request.Langua == morphPolish) {
            StripPolishAccents(query);
        }
        MakeUpperUtf8(query);
    }
    ...
    if (!P->GetSentencesFromSynAn(query, false)) {
        ...
        if (request.Langua == morphFinnish) {
            LOGW << "Finnish Syntax analysis failed (possibly due to missing dictionaries)";
            return "[]";
        }
        if (request.Langua == morphItalian) {
            LOGW << "Italian Syntax analysis failed (possibly due to missing dictionaries)";
            return "[]";
        }
        if (request.Langua == morphHungarian) {
            LOGW << "Hungarian Syntax analysis failed (possibly due to missing dictionaries)";
            return "[]";
        }
        if (request.Langua == morphPolish) {
            LOGW << "Polish Syntax analysis failed (possibly due to missing dictionaries)";
            return "[]";
        }
...
void TSynanHttpServer::LoadSynan(bool loadBigrams) {
    ...
    try {
        LOGI <<"Loading Finnish Morphology";
        FinnishMorphHolder.LoadMorphology(morphFinnish);
    } catch (CExpc& e) {
        LOGE << "Failed to load Finnish Morphology: " << e.what();
    }
    try {
        LOGI <<"Loading Finnish Syntax";
        FinnishSyntaxHolder.LoadSyntax();
    } catch (CExpc& e) {
        LOGE << "Failed to load Finnish Syntax: " << e.what();
    }
    try {
        LOGI <<"Loading Italian Morphology";
        ItalianMorphHolder.LoadMorphology(morphItalian);
    } catch (CExpc& e) {
        LOGE << "Failed to load Italian Morphology: " << e.what();
    }
    try {
        LOGI <<"Loading Italian Syntax";
        ItalianSyntaxHolder.LoadSyntax();
    } catch (CExpc& e) {
        LOGE << "Failed to load Italian Syntax: " << e.what();
    }
    try {
        LOGI <<"Loading Hungarian Morphology";
        HungarianMorphHolder.LoadMorphology(morphHungarian);
    } catch (CExpc& e) {
        LOGE << "Failed to load Hungarian Morphology: " << e.what();
    }
    try {
        LOGI <<"Loading Hungarian Syntax";
        HungarianSyntaxHolder.LoadSyntax();
    } catch (CExpc& e) {
        LOGE << "Failed to load Hungarian Syntax: " << e.what();
    }
    try {
        LOGI <<"Loading Polish Morphology";
        PolishMorphHolder.LoadMorphology(morphPolish);
    } catch (CExpc& e) {
        LOGE << "Failed to load Polish Morphology: " << e.what();
    }
    try {
        LOGI <<"Loading Polish Syntax";
        PolishSyntaxHolder.LoadSyntax();
    } catch (CExpc& e) {
        LOGE << "Failed to load Polish Syntax: " << e.what();
    }

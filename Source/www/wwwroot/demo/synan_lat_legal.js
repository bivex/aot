// ═════════════════════════════════════════════════════════════════════
//  synan_lat_legal.js — D3.js SVG visualization for Latin Legal Documents
// ═════════════════════════════════════════════════════════════════════

var SynanDaemonUrl = 'http://localhost:8089?dummy=1';
var TopClauses = [];
var CURRENT_LANG = 'Latin';

// ── Colours (Roman legal palette: imperial purple / gold / legal green) ─
var GROUP_COLOR  = '#2C3E5A';    // navy — NP/VP/PP groups
var LINK_COLOR   = '#6B2737';    // burgundy — non-group links (verb_t etc.)
var SUBJ_COLOR   = '#E3B448';    // gold/amber — subject (per requirements)
var PREDIC_COLOR = '#3CB371';    // green — predicate (per requirements)
var SP_ARC_COLOR = '#5B9BD5';    // blue — S-P arc
var FONT = '-apple-system, BlinkMacSystemFont, system-ui, sans-serif';

var SENTENCE_COLORS = [
    'rgba(27, 42, 74, 0.06)',
    'rgba(107, 39, 55, 0.06)',
    'rgba(227, 180, 72, 0.08)',
...
function getPosFromGram(g) {
    ...
    var map = {
        ...
        'INT':'INT','NUM':'NUM','PUNC':'UNKNOWN','PUNCT':'UNKNOWN','SENT':'UNKNOWN'
    };
...
function translateDescriptor(d) {
    ...
    var map = {
        'NP':'frase nom.','VP':'frase verb.','PP':'frase prep.',
        'SP':'suj-pred','VERB_T':'verbo-t',
        'ROOT':'raíz','ADJ_NP':'mod.','INF':'inf.',
        'PARTICIPLE_P':'part.'
    };
...
function parseSynanJson(json) {
    ...
    // Also try to extract morphological and legal-entity data for UI panels
    if (typeof window.extractMorphData === 'function')
        window.extractMorphData(json);
    if (typeof window.extractLegalEntities === 'function')
        window.extractLegalEntities(json);
}
...
function drawAll() {
    ...
    Object.keys(rowGroups).forEach(function(key) {
        ...
        sentG.append('rect')
            ...
            .attr('fill', SENTENCE_COLORS[ci % SENTENCE_COLORS.length])
            .attr('rx', 8).attr('ry', 8);
    });
    ...
    allSubjArcs.forEach(function(d) {
        ...
        var sTxt = 'SUBJECT', pTxt = 'PREDICATE';
        ...
        g.append('rect').attr('x', cx1 - sw2/2 - 3).attr('y', labelY - SMALL_FONT)
            ...
            .attr('fill', 'rgba(227,180,72,0.15)').attr('rx', 2);
        ...
        g.append('rect').attr('x', cx2 - pw/2 - 3).attr('y', labelY - SMALL_FONT)
            ...
            .attr('fill', 'rgba(60,179,113,0.15)').attr('rx', 2);
    ...
    var legend = svg.append('g').attr('class','legend lat-synan-legend').attr('transform','translate(14,12)');
    [
        { c: GROUP_COLOR,  d: null,  l: '— syntactic group', w: 2 },
        { c: LINK_COLOR,   d: '5,3', l: '- - link',           w: 1.5 },
        { c: SP_ARC_COLOR, d: null,  l: '⌢ subject-predicate',w: 2.5 },
...
function syntax_request() {
    ...
    if (!query) return;
    fetch(SynanDaemonUrl + '&action=syntax&langua=Latin', { method:'POST', body:query })
        .then(r => r.json())

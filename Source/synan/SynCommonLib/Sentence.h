...
{
	std::vector< std::vector<int>> m_OutputClauseVars;
protected:
...
public:
	...
	std::string GetWordsDebug(const CPeriod& P) const;
	...
	bool  GetNextClauseVariant(std::vector<size_t>& v) const;

	//read from post morph
	bool GetNextSentence(const CLemmatizedText* text, size_t& LineNo);
	virtual void	InitHomonymMorphInfo(CSynHomonym& H) = 0;

	// build
	bool CreateSyntaxStructure();

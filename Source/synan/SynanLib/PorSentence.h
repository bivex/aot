...
class CPorSentence : public CSentence
{
public:
	CPorSentence(const class CSyntaxOpt* m_pSyntaxOptions);
	~CPorSentence();
	...
	const CPorSyntaxOpt* GetOpt() const { return (const CPorSyntaxOpt*)m_pSyntaxOptions; }
	const CPorGramTab* GetPorGramTab() const { return (const CPorGramTab*)GetOpt()->GetGramTab(); }

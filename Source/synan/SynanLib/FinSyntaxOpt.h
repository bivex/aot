...
class CFinOborDic : public COborDic
{
public:
	CFinOborDic(const class CSyntaxOpt* Opt);
protected:
	bool ReadOborDic(const class CDictionary* piOborDic) override { return true; }
};

class CFinThesaurusForSyntax : public CThesaurusForSyntax
{
public:
	CFinThesaurusForSyntax(const class CSyntaxOpt* Opt) : CThesaurusForSyntax(Opt) {}
protected:
	void AssignMainGroupsToModel(class CGroups& model, const class CInnerModel& piModel) override {}
};

class CFinSyntaxOpt : public CSyntaxOpt
{
public:
	CFinSyntaxOpt(MorphLanguageEnum langua);
	void DestroyOptions();

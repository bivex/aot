...
class CFreSentence : public CSentence
{
public:
	CFreSentence(const class CSyntaxOpt* m_pSyntaxOptions);
	~CFreSentence();
	...
	const CFreSyntaxOpt* GetOpt() const { return (const CFreSyntaxOpt*)m_pSyntaxOptions; }
	const CFreGramTab* GetFreGramTab() const { return (const CFreGramTab*)GetOpt()->GetGramTab(); }

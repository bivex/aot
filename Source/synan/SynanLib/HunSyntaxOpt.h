...
class CHunOborDic : public COborDic
{
public:
	CHunOborDic(const class CSyntaxOpt* Opt);
protected:
	bool ReadOborDic(const class CDictionary* piOborDic) override { return true; }
};

class CHunThesaurusForSyntax : public CThesaurusForSyntax
{
public:
	CHunThesaurusForSyntax(const class CSyntaxOpt* Opt) : CThesaurusForSyntax(Opt) {}
protected:
	void AssignMainGroupsToModel(class CGroups& model, const class CInnerModel& piModel) override {}
};

class CHunSyntaxOpt : public CSyntaxOpt
{
public:
	CHunSyntaxOpt(MorphLanguageEnum langua);

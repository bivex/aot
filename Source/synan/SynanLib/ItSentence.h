...
class CItSentence : public CSentence
{
public:
	CItSentence(const class CSyntaxOpt* m_pSyntaxOptions);
	~CItSentence();
	...
	const CItSyntaxOpt* GetOpt() const { return (const CItSyntaxOpt*)m_pSyntaxOptions; }
	const CItaGramTab* GetItGramTab() const { return (const CItaGramTab*)GetOpt()->GetGramTab(); }

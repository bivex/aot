...
size_t  CGraphmatFile::FindOborotto(size_t FirstGraLineNo, size_t HB, const std::vector<uint16_t>& OborotIds)
{
	if (GetUnits()[FirstGraLineNo].IsGrouped()) return FirstGraLineNo;

	size_t MaxEndLineNo = FirstGraLineNo;
	auto it = m_pDicts->m_OborottosFirstWordIndex.find(OborotIds[FirstGraLineNo]);
	...
	for (auto& oborot: Oborots)
	{
		const CGraphemOborot& O = m_pDicts->m_Oborottos[oborot];
		...
		if (OborotWordNo == OborotWordsCount)
			...
			{
				GetUnit(FirstGraLineNo).SetOborot(O.m_UnitNo, O.m_bFixedFet);
